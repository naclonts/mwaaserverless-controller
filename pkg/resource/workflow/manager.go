...
// +kubebuilder:rbac:groups=mwaaserverless.services.k8s.aws,resources=workflows,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=mwaaserverless.services.k8s.aws,resources=workflows/status,verbs=get;update;patch

var lateInitializeFieldNames = []string{"LoggingConfiguration", "TriggerMode"}
...
func (rm *resourceManager) lateInitializeFromReadOneOutput(
	...
	if observedKo.Spec.TriggerMode != nil && latestKo.Spec.TriggerMode == nil {
		latestKo.Spec.TriggerMode = observedKo.Spec.TriggerMode
	}
...
func (rm *resourceManager) IsSynced(ctx context.Context, res acktypes.AWSResource) (bool, error) {
	...
	if r.ko.Status.WorkflowStatus == nil {
		return false, nil
	}
	workflowStatusCandidates := []string{"READY"}
	if !ackutil.InStrings(*r.ko.Status.WorkflowStatus, workflowStatusCandidates) {
		return false, nil
	}
